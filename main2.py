...
Read a card on a PC/SC contactless reader and TYPE its SHA1 as a keyboard.
  ...
  DESFire / other -> read the NDEF URL, scrape the card number from the
                     linked page, hash that number
...
Requires: pcscd running,
    pip install "pyscard>=2.3" evdev requests beautifulsoup4
...
# DESFire / NDEF: URL scraping settings
...
# If NDEF card-number extraction fails on a DESFire/other card, optionally fall
# back to hashing the raw UID (the old behaviour). Note the resulting digest
# will NOT match a card-number digest -- leave False unless you want that.
UID_FALLBACK = False
...
def cardnumber_preimage(card_number: str) -> bytes:
    # DESFire number comes back as a string; hash its ASCII/UTF-8 bytes so an
    # identical number reads the same as the Classic path (b"215387").
    return card_number.strip().encode("utf-8")
...
# --- DESFire / NDEF (Type 4 tag) APDUs, from the card-number solution --------
SELECT_NDEF_APP = [0x00, 0xA4, 0x04, 0x00, 0x07, 0xD2, 0x76, 0x00, 0x00, 0x85, 0x01, 0x01]
SELECT_NDEF_FILE = [0x00, 0xA4, 0x00, 0x0C, 0x02, 0xE1, 0x04]
READ_NDEF = [0x00, 0xB0, 0x00, 0x00, 0xFF]
...
# --- DESFire card-number extraction (merged from the NDEF/scrape solution) ---
def parse_ndef_uri(data):
    """Pull the first URI record out of a raw NDEF file read."""
    try:
        nlen = (data[0] << 8) | data[1]
        ndef_bytes = data[2:2 + nlen]

        for i in range(len(ndef_bytes) - 3):
            if ndef_bytes[i] == 0x55:                 # 'U' type marker
                prefix_code = ndef_bytes[i + 1]
                payload_start = i + 2
                prefix = _URI_PREFIXES.get(prefix_code, "")
                payload = "".join(
                    chr(b) for b in ndef_bytes[payload_start:] if 32 <= b < 127
                )
...
def fetch_card_number(url):
    """GET the URL and scrape the cardNumber value out of the page. -> str|None."""
    if requests is None or BeautifulSoup is None:
        print("requests/beautifulsoup4 not installed "
              "(pip install requests beautifulsoup4)", file=sys.stderr, flush=True)
        return None
    try:
        ...
        label_b = soup.find(lambda tag: tag.name == "b" and "cardNumber:" in tag.text)
        if label_b:
            label_div = label_b.find_parent("div")
            value_div = label_div.find_next_sibling("div") if label_div else None
            if value_div:
                return value_div.get_text(strip=True)

        print("could not isolate the cardNumber element in the HTML",
              file=sys.stderr, flush=True)
...
class HashTyper(CardObserver):
        ...
        for card in added:
            ...
            try:
                ...
                if kind in ("classic1k", "classic4k"):
                    ...
                    digest = self._desfire(conn)
                    if digest is None and UID_FALLBACK:
    ...
    def _desfire(self, conn):
        """Read the NDEF URL off the card, scrape the card number, hash it."""
        for apdu in (SELECT_NDEF_APP, SELECT_NDEF_FILE):
            _, sw1, sw2 = conn.transmit(apdu)
            if not _ok(sw1, sw2):
                print(f"NDEF select failed SW={sw1:02X}{sw2:02X}",
                      file=sys.stderr, flush=True)
                return None

        data, sw1, sw2 = conn.transmit(READ_NDEF)
        if not _ok(sw1, sw2):
            print(f"NDEF read failed SW={sw1:02X}{sw2:02X}",
                  file=sys.stderr, flush=True)
            return None

        url = parse_ndef_uri(data)
        if not url:
            print("could not extract a valid URL from the NDEF data",
                  file=sys.stderr, flush=True)
            return None

        card_number = fetch_card_number(url)
        if not card_number:
            return None
        return hashlib.sha1(cardnumber_preimage(card_number)).hexdigest()
